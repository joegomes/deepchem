  ...
  def build(self, graph, name_scopes, training):
    ...
    with graph.as_default():
      ...
      for ind, atomtype in enumerate(atom_types):
        ...
        for i in range(num_layers):
          weight, bias = atomicnet_ops.InitializeWeightsBiases(
                          ...
                          size=layer_sizes[i], weights=tf.truncated_normal(
                            shape=[prev_layer_size, layer_sizes[i]],
                            stddev=weight_init_stddevs[i]),
                          biases=tf.constant(value=bias_init_consts[i],
                            shape=[layer_sizes[i]]))
      ...
      def atomnet(current_input, atomtype):
        ...
        output_layer = tf.squeeze(atomicnet_ops.AtomicNNLayer(tensor=prev_layer,
                size=prev_layer_size, weights=output_weights[atomtype][0],
